...
from database import count_participants, get_latest_report, get_report, initialise, upsert_participant
...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def disable_dynamic_page_cache(response):
        if request.path.startswith(("/beheer", "/rapport", "/uitloggen")):
        ...
        response.headers["X-App-Version"] = "historische-rapporten-v2"
        return response
    ...
    def health():
        return {
            "status": "ok",
            "version": "historische-rapporten-v2",
        }, 200

    @app.get("/versie")
    def versie():
        return {
            "app_version": "historische-rapporten-v2",
            "historische_rapporten": True,
        }, 200
    ...
    @admin_required
    def rapport():
        requested_date = request.args.get("datum", "").strip()
        selected = None
        ...
        if requested_date:
            try:
                ...
                abort(400, "Ongeldige datum. Gebruik JJJJ-MM-DD.")
            selected = get_report(cfg.database_path, requested_date)
        else:
            selected = get_latest_report(cfg.database_path)

        content = (
            str(selected["content"])
            if selected
            else f"{cfg.group_name}\n\nEr werd voor deze datum nog geen rapport aangemaakt."
        )
        report_date = str(selected["report_date"]) if selected else requested_date
        whatsapp_url = f"https://wa.me/?text={quote(content, safe='')}"
        yesterday = (datetime.now().date() - timedelta(days=1)).isoformat()

        return render_template_string(
            """<!doctype html><html lang=nl><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>Dagrapport</title><style>{{style}}</style></head><body><div class=card><h1>Dagrapport</h1>{% if date %}<p class=small>Rapportdatum: {{date}}</p>{% endif %}<pre>{{content}}</pre><p><a class=button href='{{wa}}' target=_blank rel=noopener>Delen in WhatsApp</a></p><p class=small>Kies je bestaande loopgroep en druk op verzenden.</p><hr><h2>Ander rapport opvragen</h2><p><a class='button dark' href='/rapport/maken?datum={{yesterday}}'>Rapport van gisteren</a></p><form action='/rapport/maken' method=get><label for=datum>Kies een datum</label><input id=datum type=date name=datum max='{{today}}' required><button class='button orange' type=submit>Rapport maken</button></form><p><a href='/rapport/nu'>Rapport van vandaag vernieuwen</a> · <a href='/uitloggen'>Uitloggen</a></p><p class=small>Versie: historische-rapporten-v2</p></div></body></html>""",
            date=report_date,
            content=content,
            wa=whatsapp_url,
            yesterday=yesterday,
    ...
    @app.get("/rapport/maken")
    @admin_required
    def rapport_maken():
        raw_date = request.args.get("datum", "").strip()
        if not raw_date:
            abort(400, "Kies een datum.")
        ...
        if chosen_date > today:
            abort(400, "Een rapport voor een toekomstige datum is niet mogelijk.")

        run_report(cfg, chosen_date)
        return redirect(url_for("rapport", datum=chosen_date.isoformat()))
